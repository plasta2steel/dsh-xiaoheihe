...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function CommentBlock({ c, sub }) {
      ...
      return h("div", { className: "hx2-cm" + (sub ? " sub" : "") },
        ...
        h("div", { style: { flex: 1, minWidth: 0 } },
          ...
          (c.imgs || []).map((im, i) =>
            h("img", { key: i, className: "hx2-cimg", src: imgOf(im.url), alt: "", loading: "lazy", onError: (e) => (e.currentTarget.style.display = "none") }),
          ),
        ),
      );
    }

    /** 帖子正文:JSON 富文本块(img/text)→ 元素数组;纯文本 → 单段。 */
      ...
      return blocks
        .map((b, i) => {
          if (b && b.type === "img" && b.url)
            return h("img", { key: i, src: imgOf(b.url), alt: "帖子图片", loading: "lazy", onError: (e) => (e.currentTarget.style.display = "none") });
    ...
    const API = {
      feed: (offset) => rpc("/bbs/app/feeds", { offset: String(offset), pull: "0", dw: "800" }),
      ...
      search: (q, page) =>
        rpc("/bbs/app/api/general/search/v1/web", { q, search_type: "link", page: String(page), limit: "20" }),
    ...
    function Heybox() {
      ...
      const [searchRes, setSearchRes] = useState({ items: [], page: 0, loading: false, done: false });
      // messages
      ...
      useEffect(() => {
        lsSet("heybox:v2:panel", { open, width, theme });
      }, [open, width, theme]);
      ...
      const closeSearch = () => {
        ...
        setSearchQ("");
        setSearchMode(false);
        setSearchRes({ items: [], page: 0, loading: false, done: false });
      };
      const runSearch = async (q) => {
        ...
        setSearchMode(true);
        setSearchQ(q.trim());
        setSearchRes((s) => ({ ...s, items: [], page: 0, loading: true }));
        try {
          const res = await API.search(q.trim(), 1);
          const items = ((res && res.items) || []).map((it) => it.info).filter((p) => p && p.linkid);
          setSearchRes({ items, page: 1, loading: false, done: items.length < 20 });
        } catch (e) {
          runErr(e, "搜索失败");
      ...
      const moreSearch = async () => {
        if (searchRes.loading) return;
        setSearchRes((s) => ({ ...s, loading: true }));
        try {
          const next = searchRes.page + 1;
          const res = await API.search(searchQ, next);
          const items = ((res && res.items) || []).map((it) => it.info).filter((p) => p && p.linkid);
          setSearchRes((s) => ({
            items: dedupeByLinkid([...s.items, ...items]),
            page: next,
            loading: false,
      ...
      const openDetail = async (linkId, fromText) => {
        ...
        try {
          ...
          let groups = first.comments || [];
          const seenIds = new Set();
          groups.forEach((g) => { if (g.comment && g.comment[0]) seenIds.add(g.comment[0].commentid); });
          const total = first.link.comment_num || 0;
          let foldedTips = first.folded_comment_tips || "";
          let stopEmpty = 0;
          let offset = groups.length;
          const MAXC = 700; // 单帖最多拉 700 条评论,避免刷爆
          while (offset < total && offset < MAXC && stopEmpty < 3) {
            try {
              ...
              let added = 0;
              for (const g of gs) {
                if (g.comment && g.comment[0] && !seenIds.has(g.comment[0].commentid)) {
          ...
          const loadedCount = groups.reduce((s, g) => s + (g.comment ? g.comment.length : 0), 0);
          const note =
      ...
      useEffect(() => {
        ...
        const id = requestAnimationFrame(() => {
          const el = dtlScrollRef.current;
          if (!el) return;
          const saved = readPos.current[detail.linkId];
          if (saved && saved > 0) el.scrollTop = saved; // 看过:回到上次位置;新帖保持顶部
        });
        return () => cancelAnimationFrame(id);
        // eslint-disable-next-line react-hooks/exhaustive-deps
      }, [detail && detail.loading, detail && detail.linkId]);

      // 推荐流:滚到底自动加载下一页(滚动容器在列表分支挂载)。
      const onListScroll = (e) => {
        const el = e.currentTarget;
        const nearBottom = el.scrollTop + el.clientHeight >= el.scrollHeight - 160;
        if (!nearBottom) return;
        if (tab === "feed" && !searchMode && !feed.loading && !feed.noMore && feed.items.length > 0) {
          loadFeed(false);
        }
      };
      ...
      if (searchMode) {
        ...
        listEl = h("div", null,
          ...
          !searchRes.loading && searchRes.items.length === 0 && searchRes.page > 0
            ? h("div", { className: "hx2-empty" }, "没有更多结果")
            : null,
      ...
      if (!auth.ready) {
        ...
        content = h("div", { className: "hx2-detail" },
          ...
          detail.loading || !link
            ...
            : h("div", { ref: dtlScrollRef, className: "hx2-dtl", onScroll: saveReadPos },
                ...
                (link.topics || []).length
                  ...
                  : null,
                h("div", { className: "hx2-content" }, contentBlocks(link.text || link.description || "")),
                ...
                detail.comments && detail.comments.length
                  ? h("div", null,
                      detail.comments.map((g, gi) => {
                        const cs = g.comment || [];
                        if (!cs.length) return null;
                        return h("div", { key: gi, className: "hx2-cg" },
                          h(CommentBlock, { c: cs[0], sub: false }),
                          cs.slice(1).map((s) => h(CommentBlock, { key: s.commentid, c: s, sub: true })),
                        );
                      }),
                    )
        ...
        content = h("div", { ref: listScrollRef, className: "hx2-scroll", onScroll: onListScroll }, listTitle, listEl);
      }
      ...
      // 头部
      const head = h("div", { className: "hx2-head" },
        h("div", { className: "hx2-row" },
        ...
        h("div", { className: "hx2-row", style: { paddingTop: 2, paddingBottom: 7 } },
          ...
          !searchMode
            ...
            : h("button", { className: "hx2-btn", title: "刷新当前", onClick: () => { if (tab === "feed") loadFeed(true); else if (tab === "boards") { API.categories().then((r) => setBoards((r && r.latest_hot_topics && r.latest_hot_topics.children) || [])).catch((e) => runErr(e, "刷新失败")); } } }, "↻"),
        ),
        searchBar,
      );
      ...
      const showPanel = open && ((fit.fit && fit.found) || force);
      ...
      const panel = showPanel
        ? h("div", {
            ...
            style: {
              left: Math.max(8, fit.left) + "px",
              top: fit.top + "px",
              bottom: "10px",
              width: Math.max(320, Math.min(width, fit.fit ? fit.width : fit.vw - fit.left - 20)) + "px",
              "--hx2-scale": (scale / 100).toFixed(2),
            },
      ...
      const closedPill = !open
        ...
        : null;
      const crampedPill = open && !((fit.fit && fit.found) || force)
        ? h("button", { className: "hx2-pill", style: { right: "12px", top: topPill + "px" }, onClick: () => setForce(true), title: "空间不足 — 点击强制展开" },
            h("span", { className: "dot" }), "小黑盒 · 空间不足,点此展开")
        : null;
