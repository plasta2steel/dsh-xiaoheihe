  ...
  factory: (require) => {
    ...
    const CSS = `
      ...
      .hx2-pill{box-sizing:border-box;position:fixed;z-index:2147480000;display:inline-flex;align-items:center;
        ...
        background:${BASE.bg};color:${BASE.label2};font-size:12.5px;font-weight:600;cursor:grab;
        box-shadow:${BASE.shadow};user-select:none;touch-action:none;max-width:calc(100vw - 16px)}
      .hx2-pill:active{cursor:grabbing}
    ...
    function Heybox() {
      ...
      // 收起的小胶囊:左键按住拖动定位,松开停止;拖过的这次点击不触发展开。
      const pillSt = useRef(null);
      const pillMoved = useRef(false);
      const pillDown = (e) => {
        if (e.button !== 0) return;
        const b = e.currentTarget;
        const r = b.getBoundingClientRect();
        pillSt.current = { sx: e.clientX, sy: e.clientY, bx: r.left, by: r.top, moved: 0 };
        e.preventDefault();
        try {
      ...
      const pillMove = (e) => {
        const d = pillSt.current;
        if (!d) return;
        const dx = e.clientX - d.sx;
        const dy = e.clientY - d.sy;
        d.moved += Math.abs(dx) + Math.abs(dy);
        if (d.moved < 4) return;
        pillMoved.current = true;
        const vw = window.innerWidth || 1400;
        const vh = window.innerHeight || 900;
        setDragPos({
          x: Math.min(Math.max(4, d.bx + dx), Math.max(4, vw - 160)),
          y: Math.min(Math.max(4, d.by + dy), Math.max(4, vh - 60)),
        });
      };
      const pillUp = () => {
        pillSt.current = null;
      };
      const pillClickGuard = (fn) => (e) => {
        if (pillMoved.current) {
          pillMoved.current = false; // 刚拖过,忽略这次点击
          return;
        }
        fn();
      };
      ...
      const pillStyle = dragPos
        ? { left: dragPos.x + "px", top: dragPos.y + "px" }
        : { right: "12px", top: topPill + "px" };
      const pillProps = {
        className: "hx2-pill",
        style: pillStyle,
      ...
      const closedPill = !open
        ? h("button", Object.assign({}, pillProps, {
            onClick: pillClickGuard(() => setOpen(true)),
            title: "打开小黑盒(按住左键可拖动这个胶囊)",
          }), h("span", { className: "dot" }), "小黑盒" + (unread ? " · " + unread : ""))
        : null;
      const crampedPill = open && !visiblePanel && !dragPos
        ? h("button", Object.assign({}, pillProps, {
            onClick: pillClickGuard(() => setForce(true)),
            title: "空间不足 — 点此展开;也可按住左键把胶囊拖到别处,再点它展开",
    ...
    // 渲染错误兜底:把崩溃原因直接显示出来,便于排查
    class HxBoundary extends React.Component {
      constructor(props) {
        super(props);
        this.state = { err: null };
      }
      static getDerivedStateFromError(err) {
        return { err: err && err.message ? String(err.message) : String(err) };
      }
      componentDidCatch(err, info) {
        try {
          console.error("[heybox] render error:", err, info);
        } catch {
          /* ignore */
        }
      }
      render() {
        if (this.state.err) {
          return React.createElement("div", {
            style: {
              position: "fixed", right: 12, top: 12, zIndex: 2147483001,
              maxWidth: "min(480px, calc(100vw - 24px))",
              background: "#8b1e1e", color: "#fff", padding: "10px 12px",
        ...
        slots.register(
          ...
          (props) =>
            React.createElement(
              HxBoundary,
              null,
              React.createElement(Heybox, props),
            ),
        ),
      );
    };
